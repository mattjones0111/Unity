using System;
using System.Collections.Generic;
using System.Linq;
using System.Linq.Expressions;
using Api.Host.Contracts.V1;
using Marten.Events.CodeGeneration;
using Marten.Linq;

namespace Api.Host.Features.AudioItem;

public class FindByTitle : ICompiledListQuery<Contracts.V1.AudioItem, AudioItemSlim>, IQueryPlanning
{
    public required string? Title { get; set; }
    public required string Category { get; set; }
    [MartenIgnore] public int PageNumber { get; set; } = 1;
    public int SkipCount => (PageNumber - 1) * PageSize;
    public int PageSize { get; set; } = 20;

    public Expression<Func<IMartenQueryable<Contracts.V1.AudioItem>, IEnumerable<AudioItemSlim>>> QueryIs() =>
        query => query
            .Where(x => x.CategoryPaths.Contains(Category)
                        && (string.IsNullOrEmpty(Title) || x.Title.Contains(Title, StringComparison.OrdinalIgnoreCase)))
            .Skip(SkipCount)
            .Take(PageSize)
            .Select(x => new AudioItemSlim(x.Id, x.Title));

    public void SetUniqueValuesForQueryPlanning()
    {
        PageNumber = 3;
        PageSize = 20;
        Title = Guid.NewGuid().ToString();
        Category = Guid.NewGuid().ToString();
    }
}
